...
/** 第二個內建範本：原創都市・無敵流短篇，讓範本不只停留在逆境再起。 */
export const URBAN_DEMO_STORY = [
  "凌晨兩點，沈曜拖著一只舊行李箱走出海城車站。三年前，他被家族當成廢物趕出城，母親留下的債務和一張過期門禁卡，是他唯一帶回來的東西。霓虹在雨裡碎成一地光，他在街角買了杯黑咖啡，決定先找一間最便宜的公寓住下。沒有人知道，沉睡在他掌心的黑色晶片，正悄悄連上這座城市的智慧系統。",
  "第二天，他到瑞城資本應徵保全，卻在門口看見自己的名字仍掛在三年前的黑名單上。保全不敢多問，電梯監控卻在他經過時全部轉向。午休時，一封沒有寄件人的訊息出現在手機：不要查母親的死。沈曜抬眼看向高樓玻璃後的董事會，第一次明白，這座城市不是忘了他，而是有人一直害怕他回來。",
  "晶片啟動後，沈曜在地下停車場看見一行只有他能讀懂的字：城市權限，第一級。他只說了一句「查瑞城資本」，所有被刪除的交易、封存的監控與十七個假帳戶，同時在眼前展開。原來母親留下的不是債，而是一套能看穿城市資料的核心系統。沈曜換上廉價西裝，安靜走進瑞城大樓。他不是來求一份工作，而是來收回自己的名字。",
  "董事會把他當成笑話，副總裁甚至叫來保全，要他在直播鏡頭前跪下道歉。就在保全伸手的瞬間，所有大螢幕同時亮起：瑞城侵吞公益基金、操控股價、買通證人的證據，逐條出現在全城手機上。警報聲刺破會議室，車隊在樓下急煞，門外傳來記者的喊聲。副總裁臉色慘白，沈曜卻只是抬腕看了看時間：第一局，三分鐘。",
  "他沒有把瑞城一夜摧毀。沈曜先替被裁的員工追回薪資，再用一份公開報告逼董事會交出控制權。外界以為這只是幸運，直到三家競爭對手同時撤回收購，因為每一筆暗盤交易都早已被他掌握。那天傍晚，他站在最高層的落地窗前，城市霓虹像一張可以自由改寫的棋盤。黑色晶片再次提示：權限升級，未發現可匹敵對手。",
  "一週後，瑞城的新董事長由全體員工投票選出；沈曜把位置交給最懂得守住公司的人，自己只保留核心系統的最高權限。母親的冤案在全城直播中真相大白，曾經讓他滾出海城的家族，終於在門口低下頭。沈曜走過同一條霓虹街，咖啡還是苦的，風卻已經換了方向。手機上只剩最後一行字：城市已經臣服，下一個舞台是世界。"
].join("\n\n");

export const SHOWCASE_DEMO_STORIES = [DEMO_STORY, URBAN_DEMO_STORY] as const;

export const isShowcaseStory = (body: string) => SHOWCASE_DEMO_STORIES.includes(body as typeof SHOWCASE_DEMO_STORIES[number]);

export const getShowcaseNarrationPrefix = (body: string) => {
  if (body === URBAN_DEMO_STORY) return "urban-scene";
  if (body === DEMO_STORY) return "demo-scene";
  return null;
};
...
const keywordSets: Record<NarrativeMood, string[]> = {
  calm: ["平靜", "安靜", "微風", "溫柔", "日常", "咖啡", "笑", "家", "朋友", "喜歡", "擁抱", "旅行", "午後", "孩子", "車站", "街角", "公寓", "霓虹", "城市", "西裝"],
  sorrow: ["失去", "死", "傷", "離別", "遺憾", "思念", "雨", "灰燼", "孤單", "往事", "眼淚", "再見"],
  dark: ["黑", "夜", "威脅", "恐懼", "廢棄", "陰影", "秘密", "陌生", "腳步", "門後", "真相", "失蹤", "黑名單", "監控", "董事會", "害怕", "地下", "封存"],
  crisis: ["戰", "箭", "刀", "劍", "衝", "撞", "危險", "追趕", "爆炸", "警鈴", "倒數", "下降", "火災", "保全", "直播", "鏡頭", "警報", "車隊", "螢幕", "證據", "急煞"],
  rise: ["卻", "抬起頭", "重新", "不再", "理解", "原來", "終於", "醒來", "站起", "決定", "希望", "開始", "啟動", "系統", "權限", "收回", "追回", "掌握", "升級", "控制權"],
  triumph: ["勝利", "成功", "完成", "團聚", "釋然", "歡呼", "凱旋", "陽光", "明天", "真相大白", "自由", "董事長", "全體", "掌聲", "臣服", "世界", "王者", "勝出"],
};
